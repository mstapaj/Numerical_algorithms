...
plot(prepareAvg(data$Kat),prepareAvg(abs((data$Taylor_z_func_wbudowanych-data$Taylor_od_lewej_prev)/data$Taylor_z_func_wbudowanych)),xlab = "Wartości argumentów", ylab="Wartości błedu względnego",type="l",main = "Różnica między sumowaniem elementów szeregu Taylora od początku, a sumowaniem od końca (wyliczanie na podstawie poprzedniego wyrazu)",col="BLUE")
lines(prepareAvg(data$Kat),prepareAvg(abs((data$Taylor_z_func_wbudowanych-data$Taylor_od_prawej_prev)/data$Taylor_z_func_wbudowanych)),col="RED",type = "l")
legend(x="topleft", legend=c("Sumowanie od początku", "Sumowanie od końca"),
       col=c("blue", "red"), lty=c(1,1),lwd = 2)
